...
export LazyVectorTrace

"""
    LazyVectorTrace{GenFnType, T, U}

A word-for-word copy of `VectorTrace` except handles traces with no (e.g. `nothing`) generative function.
Converts into a normal `VectorTrace` when called with the GFI.
"""
struct LazyVectorTrace{GenFnType,T,U} <: Trace
    gen_fn::Any
...
function LazyVectorTrace{GenFnType,T,U}(io::IO,
                                    retval::PersistentVector{T},
    ...
    LazyVectorTrace{GenFnType,T,U}(nothing, subtraces, retval, args, len,
