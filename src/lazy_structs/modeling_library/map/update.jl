...
A word-for-word copy of `DynamicDSLTrace` except handles traces with no (e.g. `nothing`) generative function.
Converts into a normal `DynamicDSLTrace` when called with the GFI.
"""
mutable struct LazyMapUpdateState{T,U}
    weight::Float64
    score::Float64
    noise::Float64
...
function process_retained!(gen_fn::Map{T,U}, args::Tuple,
                           ...
                           state::LazyMapUpdateState{T,U}) where {T,U}
    local subtrace::U
...
function update(trace::VectorTrace{MapType,T,U}, args::Tuple, argdiffs::Tuple,
    ...
    state = LazyMapUpdateState{T,U}(-score_decrement, score, noise,
