...
mutable struct ExternalMemoryParticleFilterState{U}
    in_memory_traces::Vector{Bool}
    in_memory_new_traces::Vector{Bool}
    traces::Vector{U}
    new_traces::Vector{U}
...
function Base.copy(state::ExternalMemoryParticleFilterState{U}) where U
    return ExternalMemoryParticleFilterState{U}(
        Base.copy(state.in_memory_traces),
        Base.copy(state.in_memory_new_traces),
...
function Base.:(==)(a::ExternalMemoryParticleFilterState{U}, b::ExternalMemoryParticleFilterState{V}) where {U, V}
    return U == V &&
        a.in_memory_traces == b.in_memory_traces &&
        a.in_memory_new_traces == b.in_memory_new_traces &&
        a.traces == b.traces &&
        a.log_weights == b.log_weights &&
        a.log_ml_est == b.log_ml_est &&
        a.parents == b.parents
end
function Base.hash(state::ExternalMemoryParticleFilterState{U}, h::UInt) where {U}
    return hash(U, 
                hash(state.in_memory_traces,
                     hash(state.in_memory_new_traces,
                        hash(state.traces,
...
"""
    traces = get_traces(state::ExternalMemoryParticleFilterState)

Return the vector of traces in the current state, one for each particle.
"""
function get_traces(state::ExternalMemoryParticleFilterState)
    state.traces
end

"""
    log_weights = get_log_weights(state::ExternalMemoryParticleFilterState)
...
"""
function get_log_weights(state::ExternalMemoryParticleFilterState)
    state.log_weights
end

"""
    estimate = log_ml_estimate(state::ExternalMemoryParticleFilterState)

Return the particle filter's current estimate of the log marginal likelihood.
"""
function log_ml_estimate(state::ExternalMemoryParticleFilterState)
    num_particles = length(state.traces)
    return state.log_ml_est + logsumexp(state.log_weights) - log(num_particles)
end

"""
    traces::Vector = sample_unweighted_traces(state::ExternalMemoryParticleFilterState, num_samples::Int)

Sample a vector of `num_samples` traces from the weighted collection of traces in the given particle filter state.
"""
# EXT
function sample_unweighted_traces(state::ExternalMemoryParticleFilterState{U}, num_samples::Int) where {U}
...
"""
function initialize_external_memory_particle_filter(
    ...
    ExternalMemoryParticleFilterState{U}(traces, Vector{U}(undef, num_particles),
...
function initialize_external_memory_particle_filter(
        ...
        observations::ChoiceMap,
        num_particles::Int) where {T,U}
    traces = Vector{Union{Trace,String}}(undef, num_particles)
    in_memory_traces = Vector{Bool}(undef, num_particles)
    in_memory_new_traces = Vector{Bool}(undef, num_particles)
    log_weights = Vector{Float64}(undef, num_particles)
    for i=1:num_particles
        in_memory_traces[i] = true
        in_memory_new_traces[i] = true
        (traces[i], log_weights[i]) = generate(model, model_args, observations)
        serialize("particles/p$i.gen", traces[i])
    end
    ExternalMemoryParticleFilterState{U}(in_memory_traces, in_memory_new_traces,traces, Vector{U}(undef, num_particles),
        log_weights, 0., collect(1:num_particles))
end

"""
    (log_incremental_weights,) = particle_filter_step!(
        state::ExternalMemoryParticleFilterState, new_args::Tuple, argdiffs,
        observations::ChoiceMap, proposal::GenerativeFunction, proposal_args::Tuple)
...
function particle_filter_step!(
        state::ExternalMemoryParticleFilterState{U},
        new_args::Tuple,
        argdiffs::Tuple,
        observations::ChoiceMap,
        proposal::GenerativeFunction,
    ...
    for i=1:num_particles
        tr = realize("particles/p$i.gen", model)
...
function particle_filter_step!(
    ...
    for i=1:num_particles
        # tr = realize("particles/p$i.gen", model)
        tr = deserialize("particles/p$i.gen")
        tr.gen_fn = model
        (state.new_traces[i], increment, _, discard) = update(
            tr, new_args, argdiffs, observations)
        serialize("particles/p$i.gen", state.new_traces[i])
        # (state.new_traces[i], increment, _, discard) = update(
...
"""
    did_resample::Bool = maybe_resample!(state::ExternalMemoryParticleFilterState;
